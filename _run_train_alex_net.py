...
for epoch in range(nb_epochs):
    ...
    epoch_train_nMAP = 0
    alex_net.train()
    for i, (image_id, image, _, solar_power) in enumerate(tqdm(training_set_dataloader)):
    ...
    # Save model
    save_path = "../checkpoint/alex_net/alex_net_{}epoch.pt".format(epoch)
    save_model(epoch=epoch, model=alex_net, optim=optim, train_loss=train_loss, val_loss=val_loss, save_path=save_path)
        
    # Save learning curve
    learning_curve_save_path = "../checkpoint/alex_net/learning_curve_{}epoch".format(epoch)
